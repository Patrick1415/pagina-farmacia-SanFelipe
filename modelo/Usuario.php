...
class Usuario
{
    ...
    {
        $sql = "select * from usuario where id_usuario =:id and contraseña_us=:oldpass";
        $query = $this->acceso->prepare($sql);
        $query->execute(array(':id' => $id_usuario, ':oldpass' => $oldpass));
        $this->objetos = $query->fetchAll();
        if (!empty($this->objetos)) {
            $sql = "update usuario set contraseña_us=:newpass where id_usuario=:id";
            $query = $this->acceso->prepare($sql);
    ...
    function crear($nombre,$apellido,$dni,$edad,$pass,$tipo,$avatar){
        ...
        if(!empty($this->objetos)){
           ...
           $sql = "INSERT INTO usuario(nombre_us,apellidos_us,dni_us,edad,contraseña_us,us_tipo,avatar) VALUES (:nombre,:apellido,:dni,:edad,:pass,:tipo,:avatar);";
           $query = $this->acceso->prepare($sql);
           $query->execute(array(':nombre'=>$nombre,':apellido'=>$apellido,':dni'=>$dni,':edad'=>$edad,':pass'=>$pass,':tipo'=>$tipo,':avatar'=>$avatar));
           echo 'add';
        }
    }

    function ascender($pass, $id_ascendido, $id_usuario){
        $sql = "SELECT id_usuario FROM usuario where id_usuario=:id_usuario and contraseña_us=:pass";
        $query = $this->acceso->prepare($sql);
